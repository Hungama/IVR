...
public class MTS_Concentlog extends HttpServlet {
	...
	static HashMap hashMap = new HashMap();
		...
		public void init()
		{
			try
			{
				...
				con = DriverManager.getConnection("jdbc:mysql://10.130.14.160:3306/"+ dsn, username, pwd);
				if(con!=null)
	...
	String temp1 = "";
		try {
			...
			temp1 = "#"+service+"#" +ANI + "#" + circle + "#" + service + "#" + SC + "#" + firstconsent + "#"+firstconsenttime+"#"+secondconsent+"#"+secondconsenttime+"#"+IVRtransid+"#"+cg_id+"#"+error_desc+"#"+cons_time+"#"+cons_stat;

			FileOutputStream outp = null; // declare a file output object
			PrintStream p = null;


				File ServiceFolder = new File("/home/Hungama_call_logs/ConsentLog/MTS/"+service+"/");
				if (!ServiceFolder.exists())
				{
					ServiceFolder.mkdir();
					strlogfile = service+"_consentlog_"+strlogfile;
					outp = new FileOutputStream("/home/Hungama_call_logs/ConsentLog/MTS/"+service+"/" +strlogfile+".txt", true);
				}
				else
				{
					strlogfile = service+"_consentlog_"+strlogfile;
			...
			synchronized(this)
			{
				try
				{
					...
					stmtupdate.executeUpdate("insert into MTS_IVR.tbl_consent_log_mts(ANI,circle,service,SC,firstconsent,firstconsenttime,secondconsent,secondconsenttime,IVRtransid,cg_id,error_desc,cons_time,cons_stat,date_time)values('"+ANI+"','"+circle+"','"+service+"','"+SC+"','"+firstconsent+"','"+firstconsenttime+"','"+secondconsent+"','"+secondconsenttime+"','"+IVRtransid+"','"+cg_id+"','"+error_desc+"','"+cons_time+"','"+cons_stat+"',now())");
