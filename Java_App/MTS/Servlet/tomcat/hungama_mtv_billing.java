 ...
 */
public class hungama_mtv_billing extends HttpServlet {
	...
	public void init() {
		try {
			...
			ds = (DataSource) envCtx.lookup("jdbc/mts_hungama");
		} catch (Exception e) {
	...
	protected void processRequest(HttpServletRequest request,
        ...
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=16&ANI=9312928418&operatorid=BHAM&circleid=DEL&servicesid=3  // chk language
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=17&ANI=9312928418&DNIS=570801&LANG=HIN&operatorid=BHAM&circleid=DEL&servicesid=3  // update and insert language		
		//http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=1&ANI=111111&DNIS=1212&operatorid=BHAM&circleid=DEL&servicesid=3 // check user subscribe or not
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=2&ANI=111111&DNIS=1212&LANG=HINDI&MOS=IVR&AMT=1&operatorid=BHAM&circleid=DEL&servicesid=3 // request for subscription
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=3&ANI=111111&DNIS=1212&operatorid=BHAM&circleid=DEL&servicesid=3 // request for unsubscription
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=18&ANI=9312928418&BAL=12&DNIS=570801&operatorid=BHAM&circleid=DEL&servicesid=3  // update user balence
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=19&operatorid=BHAM&circleid=DEL&servicesid=3   // update user balence
        //http://localhost:8080/aajkahungama/hungama_mtv_billing?TOKEN=20&ANI=9312928418&DNIS=570801&songid=5657&downloadid=5787657&operatorid=BHAM&circleid=DEL&servicesid=2   // crbt
		...
		String wl_status=null,PlayListCount=null,top_id=null,operatorid=null,circleid=null,servicesid=null,songid=null,downloadid=null;
		int totaltop;
		...
		songid=(String) request.getParameter("songid");
		downloadid=(String) request.getParameter("downloadid");
		String last_cat = (String) request.getParameter("last_cat");
		String cat_flag = (String) request.getParameter("cat_flag");
		...
		try {
			...
			if (conat != null) {
				 ...
				 //out.println("connection up" +conat);
				

				if(TOKEN.equals("1")) // check user new or old 
				{
					ccstmt = conat.prepareCall("{call mts_mtv.MTV_QUERY(?,?,?,?)}");						
				...
				{
					ccstmt = conat.prepareCall("{call MTV_SUB(?,?,?,?,?,?,?)}");
				...
				{
					ccstmt = conat.prepareCall("{call MTV_UNSUB(?,?)}");
					ccstmt.setString(1, ANI);						
				...
				{
					ccstmt = conat.prepareCall("{call MTV_TOP(?,?,?)}");
					ccstmt.setString(1, ANI);		
				...
				{
					ccstmt = conat.prepareCall("{call MTV_CHK_LANG(?,?)}");						
					ccstmt.setString(1, ANI);					
				...
				{
					ccstmt = conat.prepareCall("{call MTV_LANG_CHANGE(?,?,?)}");						
					ccstmt.setString(1, ANI);
					ccstmt.setString(2, DNIS);
				...
				{
					ccstmt = conat.prepareCall("{call MTV_UPDATETIME(?,?,?)}");						
					ccstmt.setString(1, ANI);					
				...
				{
					ccstmt = conat.prepareCall("{call MTV_CHK_TOPUP(?)}");						
				...
				else if(TOKEN.equals("20")) // crbt request 
				{
					ccstmt = conat.prepareCall("{call sp_crbt_ringtone_starfollow_reqs(?,?,?,?,?)}");
					ccstmt.setInt(1,1);
					ccstmt.setString(2, ANI);												
					ccstmt.setString(3, DNIS);
					ccstmt.setString(4, songid);
					ccstmt.setString(5, downloadid);					
					ccstmt.execute();		
					System.out.println("crbt request send successfully ::"+songid);
					
				}
				else if(TOKEN.equals("21")) // set last category  
				{
					ccstmt = conat.prepareCall("{call MTV_USERPROFILE(?,?,?,?)}");						
					ccstmt.setString(1, ANI);
					ccstmt.setString(2, last_cat);
					ccstmt.setString(3, cat_flag);
					ccstmt.registerOutParameter(4,java.sql.Types.VARCHAR);				
					ccstmt.execute();					
					value = ccstmt.getString(4);
					System.out.println("lastheard cat ::"+value);
					
					String temp[]=value.split("#");
					
					last_cat = "last_cat.value='" + temp[0] + "'";
					 out.println(last_cat);
