...
import java.sql.*;
	...
	public class MTSDataProcessComedy extends Thread
	{
	...
	public void run()
	{
		try{
	            ...
	            String MU_failure = "select concat('91',msisdn) 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,res_text as 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOP-UP' when 'Event' then 'Event' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time)  from master_db.tbl_billing_failure nolock where date(response_time) =date(subdate(now(),"+day+")) and service_id=1101";
	            ...
	            String MU_unsub  = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from mts_radio.tbl_radio_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
...
//						System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				while(Rsdnis15.next())
				{
					try
					{
	                    ...
	                    TIME1 = Rsdnis15.getString("time(response_time)");
				...
				while(Rsdnis16.next())
				{
					try
					{
	                    ...
	                    TIME1 = Rsdnis16.getString("time(response_time)");
	                    //System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
						stmt_destination.executeUpdate("insert into tbl_billing_ac_mts values('"+MSISDN+"','"+MODE+"','"+CHARGINGAMOUNT+"','"+CIRCLE+"','"+CHARGINGREASON+"','"+DATE+"','"+TYPE+"','"+USER_TYPE+"','"+SERVICE+"','"+DATE1+"','"+TIME1+"')");
					}
					catch(Exception e)
				...
				while(Rsdnis17.next())
				{
					try
					{
	                    ...
	                    TIME1 = Rsdnis17.getString("time(unsub_date)");
	                    //System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
						stmt_destination.executeUpdate("insert into tbl_billing_ac_mts values('"+MSISDN+"','"+MODE+"','"+CHARGINGAMOUNT+"','"+CIRCLE+"','"+CHARGINGREASON+"','"+DATE+"','"+TYPE+"','"+USER_TYPE+"','"+SERVICE+"','"+DATE1+"','"+TIME1+"')");
					}
					catch(Exception e)
				...
				while(Rsdnis18.next())
				{
					try
					{
						...
						TIME1 = Rsdnis18.getString("time(response_time)");
				...
				while(Rsdnis19.next())
				{
					try
					{
						...
						TIME1 = Rsdnis19.getString("time(response_time)");
	            ...
	            String Contest_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from Mts_summer_contest.tbl_contest_unsub nolock where date(unsub_date) =date(subdate(now(),"+day+"))";
				...
				while(Rsdnis20.next())
				{
					try
					{
						...
						TIME1 = Rsdnis20.getString("time(unsub_date)");
	           	...
	           	String str23 = "select count(1) as CNT from tbl_billing_comedy   nolock where DATE1 = date(subdate(now()," + day + "))";
				...
				if (localResultSet23.next())
				{
					...
					{
					  System.out.println("****Deleting the records****");
					  stmt_destination.executeUpdate("delete from tbl_billing_comedy where DATE1 = date(subdate(now()," + day + "))");
					}
					else
					{
					   System.out.println("****No DB Records to Delete****");
				...
				while(Rsdnis21.next())
				{
					try
					{
						...
						TIME1 = Rsdnis21.getString("time(response_time)");
				...
				while(Rsdnis22.next())
				{
					try
					{
						...
						TIME1 = Rsdnis22.getString("time(response_time)");
	            ...
	            String Jokes_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from mts_JOKEPORTAL.tbl_jokeportal_unsub nolock where date(unsub_date) =date(subdate(now(),"+day+"))";
				...
				while(Rsdnis23.next())
				{
					try
					{
						...
						TIME1 = Rsdnis23.getString("time(unsub_date)");
	...
	public static void main(String arg[])
	{
		if(arg.length>0)
			...
			day = 1;
		MTSDataProcessComedy c = new MTSDataProcessComedy();
		c.start();
