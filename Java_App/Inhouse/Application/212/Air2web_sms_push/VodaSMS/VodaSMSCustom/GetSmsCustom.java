...
public class GetSmsCustom implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			while(true)
			{
				try
				{
				ResultSet rs2=stmt.executeQuery("select count(*)cnt from tbl_wap_sms_custom nolock ");
				if(rs2.next())
					if(rs2.getInt("cnt")>0)
					{
						ResultSet rs=stmt_update.executeQuery("select msgid,ani,message,date_time,status,dnis,circle from tbl_wap_sms_custom nolock limit 100");
						while(rs.next())
						{
						   ...
						   hu.setCircle(rs.getString("circle"));
							...
							stmt_del.executeUpdate("delete from tbl_wap_sms_custom where msgid="+msgid);
						}
					}
					else
					{
				...
				{
					System.out.println("Error at run with in loop - "+e);
					createConnect();
				}
	...
	public static void main(String[] args) 
	{
		...
		GetSmsCustom ob=new GetSmsCustom();
		ob.run();
