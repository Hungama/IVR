...
public class GetSmsMCD implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			while(true)
			{
				ResultSet rs2=stmt.executeQuery("select count(*)cnt from tbl_sendsms_tp nolock where status=0");
				if(rs2.next())
					if(rs2.getInt("cnt")>0)
					{
						ResultSet rs=stmt_update.executeQuery("select msgid,ani,message,date_time,status,dnis,circle from tbl_sendsms_tp nolock where status=0 limit 500");
						while(rs.next())
						{
                                                    try
                                                    {
						  ...
						  // hu.setCircle(rs.getString("circle"));
							...
							stmt_del.executeUpdate("delete from tbl_sendsms_tp where msgid="+msgid);
                                                    ...
                                                    {
                                                	System.out.println("Error at run exception"+e.getMessage());
							Thread.sleep(3000);
