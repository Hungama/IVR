		...
		try
		{
			...
			{	
				...
				while(aniIterator.hasNext())
				{
					...
					String str2="select ANI,circle, max(score) as cscore,count(*) as cnt, (select ifnull(max(score),0),(select SOU from tbl_contest_sou where date(date_time)=subdate(date(now()),1) and ANI='"+ANI+"') as sou, (select ifnull(chrg_amount,0) from master_db.tbl_billing_success where msisdn='"+ANI+"' and service_id=1423 and date(response_time)=subdate(date(now()),1) order by response_time limit 1) as chrg_amount  from question_played where ani='"+ANI+"' and date(date_time)<subdate(date(now()),1) order by score desc limit 1) as pscore from question_played where ani='"+ANI+"' and date(date_time)=subdate(date(now()),1) order by score desc limit 1";
					...
					while(rs2.next())
					{
						...
						int chrg_amount=Integer.parseInt(rs2.getString("chrg_amount"));
