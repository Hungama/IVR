		...
		try
		{
			...
			{	
				...
				while(aniIterator.hasNext())
				{
					...
					String str2="select ANI,circle, max(score) as cscore,count(*) as cnt, (select ifnull(max(score),0) from question_played where ani='"+ANI+"' and date(date_time)<subdate(date(now()),1) order by score desc limit 1) as pscore from question_played where ani='"+ANI+"' and date(date_time)=subdate(date(now()),1) order by score desc limit 1";
					...
					while(rs2.next())
					{
						...
						String query="insert into tbl_contest_misdaily(ANI,total_question_play,score,date_time,circle)values('"+ANI+"','"+cnt+"','"+finalscore+"',subdate(date(now()),1),'"+circle+"')";
