...
public class cRBTrNGMnger extends Thread{
...
/************ SEND SMS ************/
		...
		try
		{
			...
			while(true)
			{
				try
				{
					...
					STR = "select ani,rngid,req_type,crbt_mode,operator,status,songid,circle,DCccg_ID from tbl_crbtrng_reqs where status=7 and req_type='crbt' and DCccg_ID is not null order by date_time asc";
	...
	{
		cRBTrNGMnger crm = new cRBTrNGMnger();
		crm.start();
	}
