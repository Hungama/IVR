...
public class cRBTrNGMngerRT extends Thread{
...
//	#################################################### check_multiple METHOD  ############################################################
	 ...
	 public void check_multiple()
	 {
		 try
		 {
			 File mfile = new File("cRBTrNGMngerRT.lck");
			 if(mfile.exists())
			 {
...
/************ SEND SMS ************/
		...
		try
		{
			...
			while(true)
			{
				try
				{
					...
					STR = "select ani,rngid,req_type,crbt_mode,operator,status,songid,circle,DCccg_ID from tbl_crbtrng_reqs where status=0 and req_type!='crbt' order by date_time asc";
		            ...
		            while(rs .next())
					{
						...
						System.out.println("Picked Mobile :"+ani+" for Request "+req_type+" status is "+state+" album name "+songid+"CCGID IS :"+DCccg_ID);
						...
						{
							String smsURL1;
							smsURL1 = "http://192.168.100.211:8088/hungama/radio_cRBT?ANI="+ani+"&CLIPID="+rngid+"&TOKEN="+crbt_mode+"&OPERATOR="+operator+"&CCGID="+DCccg_ID+"";
	...
	{
		cRBTrNGMngerRT crm = new cRBTrNGMngerRT();
		crm.start();
	}
