...
import java.sql.*;
	...
	public void run()
	{
		try{
				...
				String MU_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from indicom_radio.tbl_radio_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				String Hun_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from indicom_hungama.tbl_jbox_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				String FMJ_unsub  = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from indicom_starclub.tbl_jbox_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				String RFM_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE','RedFMTataDoCoMocdma' SERVICE,date(unsub_date),time(unsub_date) from indicom_redfm.tbl_jbox_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				String Riya_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE','RiaTatadocomocdma' SERVICE,date(unsub_date),time(unsub_date) from indicom_manchala.tbl_riya_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				String MTV_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE','MTVTataDoCoMocdma' SERVICE,date(unsub_date),time(unsub_date) from indicom_hungama.tbl_mtv_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				String MND_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from indicom_mnd.tbl_character_unsub1 nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
...
/*				stmt_destination.executeUpdate("update tbl_billing_mu_tatadocomocdma set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+"))");
				...
				stmt_destination.executeUpdate("update tbl_billing_mtv set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+")) and service='MTVTataDoCoMocdma'");
				stmt_destination.executeUpdate("update tbl_billing_mtv set `CHARGING REASON`='33' where  (`CHARGING REASON` is NULL or`CHARGING REASON`='NA') and DATE1 = date(subdate(now(),"+day+")) and service='MTVTataDoCoMocdma'");
				stmt_destination.executeUpdate("update tbl_billing_mtv set `USER TYPE`='NA' where  (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL) and DATE1 = date(subdate(now(),"+day+")) and service='MTVTataDoCoMocdma'");

				stmt_destination.executeUpdate("update tbl_billing_ria set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+")) and service='RIATataDoCoMocdma'");
				stmt_destination.executeUpdate("update tbl_billing_ria set `CHARGING REASON`='33' where  (`CHARGING REASON` is NULL or`CHARGING REASON`='NA') and DATE1 = date(subdate(now(),"+day+")) and service='RIATataDoCoMocdma'");
				stmt_destination.executeUpdate("update tbl_billing_ria set `USER TYPE`='NA' where  (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL) and DATE1 = date(subdate(now(),"+day+")) and service='RIATataDoCoMocdma'");

				stmt_destination.executeUpdate("update tbl_billing_redfm set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+")) and service='RedFMTataDoCoMocdma'");
				stmt_destination.executeUpdate("update tbl_billing_redfm set `CHARGING REASON`='33' where  (`CHARGING REASON` is NULL or`CHARGING REASON`='NA') and DATE1 = date(subdate(now(),"+day+")) and service='RedFMTataDoCoMocdma'");
				stmt_destination.executeUpdate("update tbl_billing_redfm set `USER TYPE`='NA' where  (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL) and DATE1 = date(subdate(now(),"+day+")) and service='RedFMTataDoCoMocdma'");
