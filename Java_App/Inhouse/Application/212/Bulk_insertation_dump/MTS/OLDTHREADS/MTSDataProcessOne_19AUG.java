...
*/
				...
				String str1 = "select count(1) as CNT from tbl_billing_mu_mts nolock where date(Date) = date(subdate(now()," + day + "))";
				...
				if (localResultSet1.next())
				{
					...
					if (i >= 1)
					{
					  ...
					  stmt_destination.executeUpdate("delete from tbl_billing_mu_mts where date(Date) = date(subdate(now()," + day + "))");
					}
					else
					{
...
*/
				stmt_destination.executeUpdate("update tbl_billing_mu_mts set mode='IVR' where mode REGEXP '^-?[0-9]+$' and date(Date) = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_mu_mts set `CHARGING REASON`='33' where  `CHARGING REASON` is NULL or`CHARGING REASON`='NA' and date(Date) = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_mu_mts set `USER TYPE`='NA' where  `USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL and date(Date) = date(subdate(now(),"+day+"))");
