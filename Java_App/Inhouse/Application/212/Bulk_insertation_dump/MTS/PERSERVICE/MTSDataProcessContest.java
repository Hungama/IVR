...
import java.sql.*;
	...
	public class MTSDataProcessContest extends Thread
	{
...
*/
				...
				/*String Devo_Succ = "select concat('91',msisdn) 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,if(status=1,'SUCCESS','FAILURE') 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOP-UP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from master_db.tbl_billing_success nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id='1111'";
				...
				*/
				/*String MND_Success = "select concat('91',msisdn) 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,if(status=1,'SUCCESS','FAILURE') 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOP-UP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time)  from master_db.tbl_billing_success nolock where date(response_time) =date(subdate(now(),"+day+")) and service_id='1113'";
...
*/
				...
				while(Rs3dnis.next())
				{
	           	...
	           	String str2 = "select count(1) as CNT from tbl_billing_contest_mts nolock where DATE1 = date(subdate(now()," + day + "))";
	            ...
	            ResultSet Rs4dnis = stmt_source.executeQuery(Contest_Success);
	            ...
	            ResultSet Rs5dnis = stmt_source.executeQuery(Contest_failure);
				...
				ResultSet Rs6dnis = stmt_source.executeQuery(Contest_unsub);
...
*/
				...
				//stmt_destination.executeUpdate("update tbl_billing_contest_mts set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+"))");
	...
	public static void main(String arg[])
	{
		if(arg.length>0)
			...
			day = 1;
		MTSDataProcessContest c = new MTSDataProcessContest();
		c.start();
