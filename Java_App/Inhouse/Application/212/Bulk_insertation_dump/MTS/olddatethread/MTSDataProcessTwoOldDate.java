...
import java.sql.*;
	...
	public class MTSDataProcessTwoOldDate extends Thread
	{
	...
	public MTSDataProcessTwoOldDate()
	{
		try
		{
			readDBCONFIG();
...
//						System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				stmt_destination.executeUpdate("update tbl_billing_vs_mts set `USER TYPE`='NA' where  date(Date) = date(subdate(now(),"+day+")) and (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL)");
				...
				stmt_destination.executeUpdate("update tbl_billing_fmj_mts set `USER TYPE`='NA' where  date(Date) = date(subdate(now(),"+day+")) and (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL)");
				...
				stmt_destination.executeUpdate("update tbl_billing_devo_mts set `USER TYPE`='NA' where  date(Date) = date(subdate(now(),"+day+")) and (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL)");
				...
				stmt_destination.executeUpdate("update tbl_billing_54646_mts set `USER TYPE`='NA' where  date(Date) = date(subdate(now(),"+day+")) and (`USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL)");
	...
	public static void main(String arg[])
	{
		if(arg.length>0)
			...
			day = 1;
		MTSDataProcessTwoOldDate c = new MTSDataProcessTwoOldDate();
		c.start();
