...
mysql_db="reliance_radio"
mysql_user="ivr"
mysql_password="ivr"
mysql_host="192.168.100.224"
mysql_local_file="/home/java/AirmRBTrNGMnger/Script/RelMuRng.txt"

count=0
CURDATE="`$DATE +%Y-%m-%d`"

$SQL -h$mysql_host -u$mysql_user $mysql_db -p$mysql_password -e "select ani,rngid,req_type,crbt_mode,status,songid from tbl_crbtrng_reqs where status=0 and req_type!='mt'order by date_time asc;"> $mysql_local_file
...
processLine()
{
	...
	txtMSG="Thanks for downloading from Reliance MU. To download(Data Charges apply),click, http://202.87.41.147/waphung/voiceContentServe/"$rngid"/"$rndPIN
	...
	echo $ani $rngid $req_type $crbt_mode $status $songid $resp $txtMSG >> /home/java/AirmRBTrNGMnger/Script/RELMU_$CURDATE.txt
}
