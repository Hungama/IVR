...
import java.sql.*;
	...
	public class VodaDataProcess4OldDates extends Thread
	{
	...
	public VodaDataProcess4OldDates()
	{
		try
		{
			readDBCONFIG();
	...
	public void run()
	{
		try{
			...
			/*	String str1 = "select count(1) as CNT from tbl_billing_VH1 nolock where date(Date) = date(subdate(now()," + day + ")) and service='VH1Vodafone' and `CHARGING REASON`='SUCCESS'";
				ResultSet localResultSet1 = stmt_destination.executeQuery(str1);
				  if (localResultSet1.next())
				  {
					...
					if (i >= 1)
					{
					  ...
					  stmt_destination.executeUpdate("delete from tbl_billing_VH1 where date(Date) = date(subdate(now()," + day + ")) and service='VH1Vodafone' and `CHARGING REASON`='SUCCESS'");
					}
					else
					{
	            ...
	            String str2 = "select count(1) as CNT from tbl_billing_54646_vodafone nolock where date(Date) = date(subdate(now(),"+day+")) and `CHARGING REASON`='SUCCESS'";
				ResultSet localResultSet2 = stmt_destination.executeQuery(str2);
				  if (localResultSet2.next())
				  {
					...
					if (i >= 1)
					{
					  ...
					  stmt_destination.executeUpdate("delete from tbl_billing_54646_vodafone where date(Date) = date(subdate(now()," + day + "))  and `CHARGING REASON`='SUCCESS'");
					}
					else
					{
					   System.out.println("****No DB Records to Delete****");
	            ...
	            String str3 = "select count(1) as CNT from Tbl_billing_redfm nolock where date(Date) = date(subdate(now()," + day + ")) and service='RedFMVodafone' and `CHARGING REASON`='SUCCESS'";
				...
				if (localResultSet3.next())
				{
					...
					{
					  System.out.println("****Deleting the records****");
					  stmt_destination.executeUpdate("delete from Tbl_billing_redfm where date(Date) = date(subdate(now()," + day + ")) and service='RedFMVodafone' and `CHARGING REASON`='SUCCESS'");
					}
					else
					{
					   System.out.println("****No DB Records to Delete****");
...
*/
	...
	public static void main(String arg[])
	{
		if(arg.length>0)
			...
			day = 1;
		VodaDataProcess4OldDates c = new VodaDataProcess4OldDates();
		c.start();
