	...
	public void run()
	{
		try{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
	            ...
	            System.out.println("update tbl_billing_redfm set service='WAPREDFMUninor' where  TYPE='EVENT'  and service='RedFMUninor' and DATE1 = date(subdate(now()," + day + "))");
				stmt_destination.executeUpdate("update tbl_billing_redfm set service='WAPREDFMUninor' where  TYPE='EVENT'  and service='RedFMUninor' and DATE1 = date(subdate(now()," + day + "))");
				...
				stmt_destination.executeUpdate("update tbl_billing_ria set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now()," + day + "))");
				stmt_destination.executeUpdate("update tbl_billing_ria set `CHARGING REASON`='33' where  `CHARGING REASON` is NULL or`CHARGING REASON`='NA' and DATE1 = date(subdate(now()," + day + "))");
				stmt_destination.executeUpdate("update tbl_billing_ria set `USER TYPE`='NA' where  (`USER TYPE` in ('0','/N','NULL','') or `USER TYPE` is NULL) and DATE1 = date(subdate(now()," + day + "))");
