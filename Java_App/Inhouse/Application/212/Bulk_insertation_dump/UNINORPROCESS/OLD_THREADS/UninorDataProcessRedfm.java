...
import java.sql.*;
	...
	public class UninorDataProcessRedfm extends Thread
	{
	...
	public void run()
	{
		try{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
	            ...
	            String Ring_Failure = "select concat('91',msisdn) 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,status as 'CHARGING REASON',response_time 'DATE',case plan_id when '69' then 'Polytunes' when '70' then 'Monotones' when '71' then 'TrueTones' else 'others' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from master_db.tbl_billing_failure nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=1412";
	...
	public static void main(String arg[])
	{
		if(arg.length>0)
			...
			day = 1;
		UninorDataProcessRedfm c = new UninorDataProcessRedfm();
		c.start();
