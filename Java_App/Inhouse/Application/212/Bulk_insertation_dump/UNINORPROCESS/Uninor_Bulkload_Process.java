...
import java.sql.*;
	...
	public class Uninor_Bulkload_Process extends Thread
	{
	...
	public Uninor_Bulkload_Process()
	{
		try
		{
			readDBCONFIG();
	...
	public void CRIBulkLoad(String filepath)
	{
		try
		{
	            String str8 = "select count(1) as CNT from tbl_billing_cri_uninor nolock where DATE1 = date(subdate(now()," + day + "))";
	...
	public void VABulkLoad(String filepath)
	{
		try
		{
	            String str11 = "select count(1) as CNT from tbl_billing_va_uninor nolock where DATE1 = date(subdate(now()," + day + "))";
				...
				if (localResultSet11.next())
				{
					...
					{
					   System.out.println("****No DB Records to Delete for VA****");
					}
				}
				System.out.println("***********UNINOR VA DATA INSERTING AT DESTINATION***********");
				Date Current_Date = new Date();
				System.out.println(String.format("Current Date/Time : %tc", Current_Date ));
						String esquel = "LOAD DATA LOCAL INFILE '" + filepath +
												"' INTO TABLE tbl_billing_va_uninor " +
												" FIELDS TERMINATED BY \'#\'" +
												" LINES TERMINATED BY \'\\n\'";
						stmt_destination.executeUpdate(esquel);
				System.out.println("***********UNINOR VA DATA INSERTED AT DESTINATION***********");
				Date Current_Date1 = new Date();
				System.out.println(String.format("Current Date/Time : %tc", Current_Date1 ));
		}
		catch(Exception e)
	...
	public void SMSUNIMBulkLoad(String filepath)
	{
		try
		{
	            String str11 = "select count(1) as CNT from tbl_billing_sms_uninor nolock where DATE1 = date(subdate(now()," + day + "))";
				ResultSet localResultSet11 = stmt_destination.executeQuery(str11);
				if (localResultSet11.next())
				{
					i = localResultSet11.getInt("CNT");
					...
					{
					   System.out.println("****No DB Records to Delete for SMS****");
					}
				}
				System.out.println("***********UNINOR SMS DATA INSERTING AT DESTINATION***********");
				Date Current_Date = new Date();
				System.out.println(String.format("Current Date/Time : %tc", Current_Date ));
						String esquel = "LOAD DATA LOCAL INFILE '" + filepath +
												"' INTO TABLE tbl_billing_sms_uninor " +
												" FIELDS TERMINATED BY \'#\'" +
												" LINES TERMINATED BY \'\\n\'";
						stmt_destination.executeUpdate(esquel);
				System.out.println("***********UNINOR SMS DATA INSERTED AT DESTINATION***********");
				Date Current_Date1 = new Date();
				System.out.println(String.format("Current Date/Time : %tc", Current_Date1 ));
		}
		catch(Exception e)
	...
	public void REGUNIMBulkLoad(String filepath)
	{
		try
		{
	            String str11 = "select count(1) as CNT from tbl_billing_reg_uninor nolock where DATE1 = date(subdate(now()," + day + "))";
				ResultSet localResultSet11 = stmt_destination.executeQuery(str11);
				if (localResultSet11.next())
				{
					i = localResultSet11.getInt("CNT");
					...
					{
					  System.out.println("****Deleting the records****");
					  stmt_destination.executeUpdate("delete from tbl_billing_reg_uninor where DATE1 = date(subdate(now()," + day + "))");
					}
					else
					{
					   System.out.println("****No DB Records to Delete for SMS****");
				...
				System.out.println("***********UNINOR REG DATA INSERTING AT DESTINATION***********");
				Date Current_Date = new Date();
				System.out.println(String.format("Current Date/Time : %tc", Current_Date ));
						String esquel = "LOAD DATA LOCAL INFILE '" + filepath +
												"' INTO TABLE tbl_billing_reg_uninor " +
												" FIELDS TERMINATED BY \'#\'" +
												" LINES TERMINATED BY \'\\n\'";
						stmt_destination.executeUpdate(esquel);
				System.out.println("***********UNINOR REG DATA INSERTED AT DESTINATION***********");
				Date Current_Date1 = new Date();
				System.out.println(String.format("Current Date/Time : %tc", Current_Date1 ));
		}
		catch(Exception e)
	...
	public void run()
	{
		try{
				...
				VABulkLoad("/home/MISDATA_TEST/BILLINGDATA/UNIM/Uninor_VA_"+ FILEDATE + ".txt");
				SMSUNIMBulkLoad("/home/MISDATA_TEST/BILLINGDATA/UNIM/Uninor_SMS_"+ FILEDATE + ".txt");
				REGUNIMBulkLoad("/home/MISDATA_TEST/BILLINGDATA/UNIM/Uninor_REG_"+ FILEDATE + ".txt");
			...
			public static void main(String arg[])
			{
				if(arg.length>0)
					...
					day = 1;
				Uninor_Bulkload_Process c = new Uninor_Bulkload_Process();
				c.start();
