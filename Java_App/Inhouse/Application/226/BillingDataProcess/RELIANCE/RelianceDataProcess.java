...
import java.sql.*;
	...
	/*******************MAKE TEST FILES********************/
		public void Fun_FileCreator(int Day)
		{
				try
				{
					...
					cstmtfm1 = con_destination.prepareCall("{call misdata.Reliance_Billing_Data_All(?)}");
	...
	public void run()
	{
		try{
				...
				String RMM_Success = "select concat('91',msisdn) 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,if(status=1,'SUCCESS','FAILURE') 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOP-UP' when 'EVENT' then 'EVENT' when 'resub' then 'Renewal' else event_type end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),ccg_id from "+ BillingTable+" nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=1201";
				String RMM_failure = "select concat('91',msisdn) 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,status as 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOP-UP' when 'EVENT' then 'EVENT' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),ccg_id from master_db.tbl_billing_failure nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=1201";
				String RMM_unsub   = "select concat('91',ani) 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT',circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from reliance_music_mania.tbl_MusicMania_unsub nolock where date(unsub_date)=date(subdate(now(),"+day+"))";
				...
				stmt_destination.executeUpdate("truncate table tbl_billing_54646_reliance");
				...
				stmt_destination.executeUpdate("truncate table tbl_billing_mtv_reliance");
				...
				stmt_destination.executeUpdate("truncate table tbl_billing_cri_reliance");
				...
				stmt_destination.executeUpdate("truncate table tbl_billing_mod_reliance");
				ResultSet Rs20dnis = stmt_source.executeQuery(RMM_Success);
				System.out.println("***********RELIANCE RMM SUCCESS TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs20dnis.next())
				{
					try
					{
						...
						//System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
						stmt_destination.executeUpdate("insert into tbl_billing_mod_reliance values('"+MSISDN+"','"+MODE+"','"+CHARGINGAMOUNT+"','"+CIRCLE+"','"+CHARGINGREASON+"','"+DATE+"','"+TYPE+"','"+USER_TYPE+"','"+SERVICE+"','"+DATE1+"','"+TIME1+"')");
					}
					catch(Exception e)
					{
						System.out.println(e);
				...
				ResultSet Rs21dnis = stmt_source.executeQuery(RMM_failure);
				System.out.println("***********RELIANCE RMM FAILURE TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs21dnis.next())
				{
					try
					{
						...
						DATE1 = Rs21dnis.getString("date(response_time)");
						TIME1 = Rs21dnis.getString("ccg_id");
						//System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
						stmt_destination.executeUpdate("insert into tbl_billing_mod_reliance values('"+MSISDN+"','"+MODE+"','"+CHARGINGAMOUNT+"','"+CIRCLE+"','"+CHARGINGREASON+"','"+DATE+"','"+TYPE+"','"+USER_TYPE+"','"+SERVICE+"','"+DATE1+"','"+TIME1+"')");
					}
					catch(Exception e)
				...
				ResultSet Rs22dnis = stmt_source.executeQuery(RMM_unsub);
				System.out.println("***********RELIANCE RMM UNSUB TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs22dnis.next())
				{
					try
					{
						...
						DATE1 = Rs22dnis.getString("date(unsub_date)");
						TIME1 = Rs22dnis.getString("time(unsub_date)");
	                    TIME1 = "X";
				...
				stmt_destination.executeUpdate("update tbl_billing_cri_reliance set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_cri_reliance set `CHARGING REASON`='33' where  `CHARGING REASON` is NULL or`CHARGING REASON`='NA' and DATE1 = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_cri_reliance set `USER TYPE`='NA' where  `USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL and DATE1 = date(subdate(now(),"+day+"))");

				stmt_destination.executeUpdate("update tbl_billing_mtv_reliance set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_mtv_reliance set `CHARGING REASON`='33' where  `CHARGING REASON` is NULL or`CHARGING REASON`='NA' and DATE1 = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_mtv_reliance set `USER TYPE`='NA' where  `USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL and DATE1 = date(subdate(now(),"+day+"))");

				stmt_destination.executeUpdate("update tbl_billing_mod_reliance set mode='IVR' where mode REGEXP '^-?[0-9]+$' and DATE1 = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_mod_reliance set `CHARGING REASON`='33' where  `CHARGING REASON` is NULL or`CHARGING REASON`='NA' and DATE1 = date(subdate(now(),"+day+"))");
				stmt_destination.executeUpdate("update tbl_billing_mod_reliance set `USER TYPE`='NA' where  `USER TYPE` in ('0','/N','NULL') or `USER TYPE` is NULL and DATE1 = date(subdate(now(),"+day+"))");
