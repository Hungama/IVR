...
import java.sql.*;
	...
	/************ MAKE TEST FILES ************/
		public void Fun_FileCreator(int Day)
		{
				try
				{
					...
					cstmtfm1 = con_destination.prepareCall("{call misdata.Etisalat_Billing_Data_All(?)}");
	...
	public void run()
	{
		try{
				...
				String CCP_Success = "select msisdn 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','CCP' circle,if(status=1,'SUCCESS','FAILURE') 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOPUP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from "+BillingTable+" nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=2121 and Plan_id in(244,245,246)";
				String CCP_failure = "select msisdn 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','CCP' circle,status as 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOPUP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from master_db.tbl_billing_failure nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=2121 and Plan_id in(244,245,246)";
				String CCP_unsub   = "select ani 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','CCP' circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from etislat_hsep.tbl_ccp_subscription_log nolock where date(unsub_date)=date(subdate(now(),"+day+")) and Plan_id in(244,245,246)";

				String DAP_Success = "select msisdn 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','DAP' circle,if(status=1,'SUCCESS','FAILURE') 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOPUP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from "+BillingTable+" nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=2121 and Plan_id in(247,248,249)";
				String DAP_failure = "select msisdn 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','DAP' circle,status as 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOPUP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from master_db.tbl_billing_failure nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=2121 and Plan_id in(247,248,249)";
				String DAP_unsub   = "select ani 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','DAP' circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from etislat_hsep.tbl_dap_subscription_log nolock where date(unsub_date)=date(subdate(now(),"+day+")) and Plan_id in(247,248,249)";

				String HBP_Success = "select msisdn 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','HBP' circle,if(status=1,'SUCCESS','FAILURE') 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOPUP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from "+BillingTable+" nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=2121 and Plan_id in(250,251,252)";
				String HBP_failure = "select msisdn 'MSISDN',if(mode is null,'IVR',mode) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','HBP' circle,status as 'CHARGING REASON',response_time 'DATE',case event_type when 'sub' then 'Activation' when 'TOPUP' then 'TOPUP' else 'Renewal' end 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(response_time),time(response_time) from master_db.tbl_billing_failure nolock where date(response_time)=date(subdate(now(),"+day+")) and service_id=2121 and Plan_id in(250,251,252)";
				String HBP_unsub   = "select ani 'MSISDN',if(unsub_reason is null,'IVR',unsub_reason) 'MODE',if(chrg_amount is null,'0',chrg_amount) 'CHARGING AMOUNT','HBP' circle,'SUCCESS' as 'CHARGING REASON',unsub_date 'DATE','Deactivation' as 'TYPE',pre_post 'USER_TYPE',plan_id 'SERVICE',date(unsub_date),time(unsub_date) from etislat_hsep.tbl_hbp_subscription_log nolock where date(unsub_date)=date(subdate(now(),"+day+")) and Plan_id in(250,251,252)";
				...
				stmt_destination.executeUpdate("truncate table tbl_billing_sms_etisalat");
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs25dnis = stmt_source.executeQuery(CCP_Success);
				System.out.println("***********ETISALAT CCP SUCCESS TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs25dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs26dnis = stmt_source.executeQuery(CCP_failure);
				System.out.println("***********ETISALAT CCP FAILURE TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs26dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs27dnis = stmt_source.executeQuery(CCP_unsub);
				System.out.println("***********ETISALAT CCP UNSUB TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs27dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs28dnis = stmt_source.executeQuery(DAP_Success);
				System.out.println("***********ETISALAT DAP SUCCESS TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs28dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs29dnis = stmt_source.executeQuery(DAP_failure);
				System.out.println("***********ETISALAT DAP FAILURE TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs29dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs30dnis = stmt_source.executeQuery(DAP_unsub);
				System.out.println("***********ETISALAT DAP UNSUB TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs30dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs31dnis = stmt_source.executeQuery(HBP_Success);
				System.out.println("***********ETISALAT HBP SUCCESS TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs31dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs32dnis = stmt_source.executeQuery(HBP_failure);
				System.out.println("***********ETISALAT HBP FAILURE TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs32dnis.next())
				{
					try
					{
			...
			//			System.out.println(MSISDN+","+MODE+","+CHARGINGAMOUNT+","+CIRCLE+","+CHARGINGREASON+","+DATE+","+TYPE+","+USER_TYPE+","+SERVICE+","+DATE1+","+TIME1);
				...
				ResultSet Rs33dnis = stmt_source.executeQuery(HBP_unsub);
				System.out.println("***********ETISALAT HBP UNSUB TABLE DATA INSERTING AT DESTINATION***********");
				while(Rs33dnis.next())
				{
					try
					{
