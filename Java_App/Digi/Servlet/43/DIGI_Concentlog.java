...
public class DIGI_Concentlog extends HttpServlet {
	...
	static HashMap hashMap = new HashMap();
		...
		public static String dsn="dm_radio";
		...
		public void init()
		{
			try
			{
				...
				con = DriverManager.getConnection("jdbc:mysql://172.28.106.4:3306/"+ dsn, username, pwd);
				if(con!=null)
	...
	String temp1 = "";
		try {
			...
			PrintStream p = null;
				...
				File ServiceFolder = new File("/home/Hungama_call_logs/ConsentLog/DIGI/"+service+"/");
				if (!ServiceFolder.exists())
				{
					ServiceFolder.mkdir();
					strlogfile = service+"_consentlog_"+strlogfile;
					outp = new FileOutputStream("/home/Hungama_call_logs/ConsentLog/DIGI/"+service+"/" +strlogfile+".txt", true);
				}
				else
				{
					strlogfile = service+"_consentlog_"+strlogfile;
			...
			synchronized(this)
			{
				try
				{
					...
					stmtupdate.executeUpdate("insert into Digi_IVR.tbl_consent_log_digi(ANI,circle,service,SC,firstconsent,firstconsenttime,secondconsent,secondconsenttime,date_time)values('"+ANI+"','"+circle+"','"+service+"','"+SC+"','"+firstconsent+"','"+firstconsenttime+"','"+secondconsent+"','"+secondconsenttime+"',now())");
